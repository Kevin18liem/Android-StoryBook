...
public class P4_Draggable : MonoBehaviour {
		...
		if (moving) {
			if (snap) {
				if (Vector3.Distance (transform.position, target.position) >= dDist) {
					...
					moveable = false;
					if (succ) {
						succ.GetComponent<P4_Draggable> ().moveable = true;
						succ.GetComponent<P4_Draggable> ().putHintHere();
					}
				}
			} else {
	...
	public void putHintHere() {
		hint.transform.position = new Vector3 (initPos.x + 1, initPos.y - 3, initPos.z);
		hint.SetActive (true);
	}
