...
public class P8_Subtitle : MonoBehaviour {
	...
	IEnumerator Spell (float sec) {
		...
		if (idx == texts[wordset].words.Length) {
			subAllowed = false;
			holder.GetComponent<P8_SubtitleHolder> ().allowClick = true;
			seqManager.GetComponent<P8_SequenceManager> ().inSequence = false;
			seqManager.GetComponent<P8_SequenceManager> ().sequence++;
			anakSprite.GetComponent<Animator> ().SetTrigger ("idle");
		}
	}
